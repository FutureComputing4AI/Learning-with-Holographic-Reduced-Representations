...
class XMLDataset(Dataset):
    def __init__(self, params, train=True):
        ...
        self.end_token = '<END/>'
        self.train = train
        self.hrr_labels = None
        self._use_hrr = params.hrr_labels

        # Load the preprocessed datasets.
        if self.train:
            self.features = sparse.load_npz(params.data_path + '/x_train.npz')
            self.labels = sparse.load_npz(params.data_path + '/y_train.npz')
            if self._use_hrr:
                self.hrr_labels = sparse.load_npz(params.data_path + '/y_train_hrr.npz')
        else:
            self.features = sparse.load_npz(params.data_path + '/x_test.npz')
            self.labels = sparse.load_npz(params.data_path + '/y_test.npz')
            if self._use_hrr:
                self.hrr_labels = sparse.load_npz(params.data_path + '/y_test_hrr.npz')

        self.vocab = np.load(params.data_path + '/vocab.npy', allow_pickle=True).item()
        self.vocab_inv = np.load(params.data_path + '/vocab_inv.npy')
        self.num_features = self.features.shape[1]
        self.num_labels = self.labels.shape[1]

        # Check if the number of samples are the same for features and labels.
        assert self.features.shape[0] == self.labels.shape[0], "Mismatch between features and labels!"
        self.num_samples = self.features.shape[0]
        self.vocab_size = len(self.vocab)

        # Params Update.
        params.X_dim = self.features.shape[1]
        params.y_dim = self.labels.shape[1]
        params.N = self.num_samples
        params.vocab_size = self.vocab_size
        params.classes = params.y_dim
        params.vocabulary_inv = self.vocab_inv
    ...
    def __getitem__(self, idx):
        ...
        x = self.features[idx].toarray()
        if self._use_hrr and self.train:
            y_hrr = self.hrr_labels[idx].toarray()
            return np.squeeze(x, axis=0), np.squeeze(y_hrr, axis=0)
        else:
            y = self.labels[idx].toarray()
...
def load_batch_cnn(x_tr, y_tr, params, batch=True, batch_size=0,
                   ...
                   testing=0, y_tr_hrr=None, return_labels=True):
