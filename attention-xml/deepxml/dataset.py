...
class MultiLabelDataset(Dataset):
    ...
    def __init__(self, data_x: TDataX, data_y: TDataY = None,
                 spn_data_y: TDataZ = None, training=True):
        self.data_x, self.data_y, self.training = data_x, data_y, training
        self.spn_data_y = spn_data_y
    ...
    def __getitem__(self, item):
        ...
        if self.training and self.data_y is not None:
            ...
            if self.spn_data_y is not None:
                spn_data_y = self.spn_data_y[item].toarray().squeeze(0).astype(np.long)
                return data_x, data_y, spn_data_y
            else:
