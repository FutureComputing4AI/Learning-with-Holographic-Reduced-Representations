...
class MLAttention(nn.Module):
    ...
    def forward(self, inputs, masks):
        ...
        output = output.mean(dim=1) # Take a mean of the vectors across axis=1.
        return output   # N, labels_num, hidden_size
...
class MLLinear(nn.Module):
    ...
    def __init__(self, linear_size, output_size):
        ...
        self.linear = nn.ModuleList(nn.Linear(in_s, out_s)
                                    for in_s, out_s in zip(linear_size[:-1],
                                                           linear_size[1:]))
        # Initialize each layer.
        for linear in self.linear:
            nn.init.xavier_uniform_(linear.weight)

        # NOTE: In the original architecture:
        #       Output size is 1.
        #       Giving an output of x, 1 tensor.

        # New Architecture: output size is label size.
