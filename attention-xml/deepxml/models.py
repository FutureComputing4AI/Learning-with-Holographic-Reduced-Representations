...
class Model(object):
    ...
    def __init__(self, network, model_path, gradient_clip_value=5.0, device_ids=None, **kwargs):
        ...
        if self.model.module.use_spn:
            self.loss_fn = self.model.module.spp_loss
        else:
    ...
    def train_step(self, train_x: torch.Tensor, train_y: torch.Tensor):
        ...
        loss.backward()
        if self.model.module.use_spn == False:
            self.clip_gradient()
    ...
    def predict_step(self, data_x: torch.Tensor, k: int):
        ...
        with torch.no_grad():
            if self.model.module.use_spn:
                s = self.model(data_x)
                y = self.model.module.inference(s, s.shape[0])
                weights = self.model.module.class_vec.weight.t()
                all_scores = torch.abs(torch.mm(y, weights)[:,:-1])

                scores, labels = torch.topk(all_scores, k)
    ...
    def train(self, train_loader: DataLoader, valid_loader: DataLoader, opt_params: Optional[Mapping] = None,
        ...
        for epoch_idx in range(nb_epoch):
            ...
            for i, data in enumerate(train_loader, 1):
                ...
                if self.model.module.use_spn:
                    spn_train_y = data[2]
                    loss = self.train_step(train_x, spn_train_y.cuda().long())
                else:
