...
def build_vocab(texts: Iterable, w2v_model: Union[KeyedVectors, str], vocab_size=500000,
    ...
    if isinstance(w2v_model, str):
        # w2v_model = KeyedVectors.load(w2v_model, binary=True)
...
def build_spn_labels(labels, split_idx=None, label_idx=None, max_len=0):
    ...
    for label_row in labels:
        ...
        for l in label_row:
            ...
            spn_label_row.append(label_idx[l])

        spn_labels.append(spn_label_row)
        if len(spn_label_row) > max_len:
            max_len = len(spn_label_row)

    # Add a END of set label for SPN.
    label_idx["SPN"] = idx
    idx += 1

    # Create a constant size matrix.
    # NOTE: This is for SPN label set.
    for i, row in enumerate(spn_labels):
        row = row + [label_idx["SPN"] for i in range(0, max_len - len(row))]
        spn_labels[i] = row

    spn_labels = sparse.csr_matrix(np.asarray(spn_labels))
    return label_idx, spn_labels[0: split_idx], spn_labels[split_idx: ], idx


def convert_to_spn(labels):
    sum = labels.sum(axis=1)
    max_len = sum.max()

    spn_labels = [[] for i in range(labels.shape[0])]
    row, col = labels.nonzero()
    for i in range(row.shape[0]):
        spn_labels[row[i]].append(col[i])

    # Create a constant size matrix.
    # NOTE: This is for SPN label set.
    for i, row in enumerate(spn_labels):
        row = row + [labels.shape[1] for i in range(0, max_len - len(row))]
        spn_labels[i] = row
...
def get_mlb(mlb_path, labels=None) -> MultiLabelBinarizer:
    ...
    mlb = MultiLabelBinarizer(sparse_output=True) # Create a binarizer if one has not been created before.
    mlb.fit(labels)
